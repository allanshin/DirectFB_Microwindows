...
/*blend value used for MWROP_BLENDTRANSPARENCY bitblt operation*/
static int mwAlphaValue = 150;

void SetAlphaValue(int alphaValue)
{
	mwAlphaValue = alphaValue;
}

int GetAlphaValue(void)
{
	return (mwAlphaValue & 0xff);
}
	...
	PSD srcpsd, MWCOORD srcx, MWCOORD srcy, long op)
{
	...
	if((op & MWROP_EXTENSION) == MWROP_SRCTRANSCOPY)
	{
		opB = op & 0xff;
		opG = (op >> 8) & 0xff;
		opR = (op >> 16) & 0xff;
	...
	if((op & MWROP_EXTENSION) == MWROP_BLENDTRANS)
	{
		alpha = GetAlphaValue();
		opB = op & 0xff;
		opG = (op >> 8) & 0xff;
