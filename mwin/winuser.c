...
#include "wintern.h"
#include "device.h"
#include "winlayered.h"
#include <stdlib.h>
#include <string.h>
#include <directfb.h>
#include <stdio.h>

extern IDirectFBDisplayLayer *DFB_layer;
...
SendMessage(HWND hwnd, UINT Msg,WPARAM wParam,LPARAM lParam)
{
	if (!IsWindow(hwnd))
	{
		printf("SendMessage: Not a window hwnd=0x%x, msg=0x%x, wParam=%d, lParam=%d\n", hwnd, Msg, wParam, lParam);
		return 0;
	}
	...
	UINT wRemoveMsg)
{
	...
	if(mwMsgHead.head == NULL) {
		...
		Update screen before clearing update regions, since
		screen update is higher priority*/
		MwUpdateScreenFromBuffer();
#endif
...
DispatchMessage(CONST MSG *lpMsg)
{
	...
	{
		return SendPaintMessage(lpMsg->hwnd);
	}
	...
	HWND hwndParent, HMENU hMenu, HINSTANCE hInstance, LPVOID lpParam)
{
	...
	wp->id = (int)hMenu;
	wp->gotPaintMsg = PAINT_PAINTED;
	wp->gotClearRegionMsg = REGION_CLEARED;
	...
	if (pwp == rootwp)
	{
		...
		wp->DFB_winrect.left = desc.posx = pwp->DFB_winrect.left + x;
		wp->DFB_winrect.top = desc.posy = pwp->DFB_winrect.top + y;
		wp->DFB_winrect.right = wp->DFB_winrect.left + nWidth;
		wp->DFB_winrect.bottom = wp->DFB_winrect.top + nHeight;
		desc.width  = nWidth;
		desc.height = nHeight;
	...
	#undef CreateWindow
#endif
		DFBCHECK( DFB_layer->CreateWindow( DFB_layer, &desc, &wp->DFB_window));
		wp->DFB_window->GetSurface(wp->DFB_window, &wp->DFB_windowSurface);


		wp->DFB_windowSurface->Lock(wp->DFB_windowSurface, DSLF_WRITE, &wp->DFB_addr, &wp->DFB_linelen);
		wp->DFB_windowSurface->Unlock(wp->DFB_windowSurface);

		wp->DFB_windowSurface->GetPixelFormat(wp->DFB_windowSurface, &format_of_the_surface);
		bpp = DFB_BITS_PER_PIXEL(format_of_the_surface);
		if (bpp > 8)
		{
			wp->DFB_linelen = wp->DFB_linelen / (bpp / 8);
		}	

        wp->DFB_windowSurface->SetColor( wp->DFB_windowSurface, 0x00, 0x00, 0x00, 0x00 );
		wp->DFB_windowSurface->FillRectangle( wp->DFB_windowSurface, 0, 0, desc.width, desc.height );

		

/*
          DFBCHECK(wp->DFB_windowSurface->GetSize( wp->DFB_windowSurface, &xres, &yres ));	    
		  wp->DFB_windowSurface->GetPixelFormat(wp->DFB_windowSurface, &format_of_the_surface);
...
*/
#if 0
	{
			DFBWindowOptions options = DWOP_COLORKEYING | DWOP_ALPHACHANNEL;

			
			/*enable colorkeying*/
			wp->DFB_window->SetOptions(wp->DFB_window, options);
			
			/*set the transparent color key */
			wp->DFB_window->SetColorKey(wp->DFB_window, 0xff, 0x00, 0x00);
		}
#endif
		SetLayeredWindowAttributes(wp, 0, 0xff, 0);		
	...
	{
		/*child window should use subsurface */
		int bpp = 0;
		DFBSurfacePixelFormat format_of_the_surface;
		DFBRectangle rect = {x, y, nWidth, nHeight};


		wp->DFB_winrect.left = pwp->DFB_winrect.left + x;
		wp->DFB_winrect.top = pwp->DFB_winrect.top + y;
		wp->DFB_winrect.right = wp->DFB_winrect.left + nWidth;
		wp->DFB_winrect.bottom = wp->DFB_winrect.left + nHeight;
		wp->DFB_window = pwp->DFB_window;
		pwp->DFB_windowSurface->GetSubSurface (pwp->DFB_windowSurface, &rect, &wp->DFB_windowSurface);
		wp->DFB_windowSurface->Lock( wp->DFB_windowSurface, DSLF_WRITE , &wp->DFB_addr, &wp->DFB_linelen);
		wp->DFB_windowSurface->Unlock( wp->DFB_windowSurface);
		wp->DFB_windowSurface->GetPixelFormat(wp->DFB_windowSurface, &format_of_the_surface);
		bpp = DFB_BITS_PER_PIXEL(format_of_the_surface);
		...
		/*Assign layered windows properties same as parent*/
		if (pwp->exstyle & WS_EX_LAYERED)
		{
			wp->exstyle |= WS_EX_LAYERED;
		}
		wp->crKey = pwp->crKey;
		wp->bAlpha = pwp->bAlpha;
		wp->dwFlags = pwp->dwFlags;
	...
	if(wp->style & WS_VISIBLE) {
		...
		/* ashin: Removed setfocus because WB control windows conflict
		 * when window newly created/destroyed from WB state machine, 
		 * but activate/deactivate focus messages sent to window causing crash
		 */
		//SetFocus(wp);
	}
//printf("created window =x%x, rootwp = 0x%x\n", wp, rootwp);
	return wp;
}
...
MwDestroyWindow(HWND hwnd,BOOL bSendMsg)
{
	...
	{
		hwnd->DFB_windowSurface->Release(hwnd->DFB_windowSurface);
		hwnd->DFB_window->Release(hwnd->DFB_window);			
	}
	else
	{
		hwnd->DFB_windowSurface->Release(hwnd->DFB_windowSurface);		
	...
	UINT fuFlags)
{
	...
	if(bZorder) {
		...
		case (int)HWND_BOTTOM:
			...
			break;
		default:
			MwInsertWindow(hwnd, hwndInsertAfter);
			break;
		}
	} else {
		if(!hidden)
...
/******************************************************************************
 * ashin 11/04/05: Timer Code to support any number of timers,
 * Singly linked list implementation.
 ******************************************************************************/
typedef struct TIMER* PTIMER;

typedef struct TIMER {			/* private timer structure*/
	...
	PTIMER	nextTimer;		/* pointer to next timer in list*/
} TIMER;

static TIMER *timerList = NULL;	/* pointer to head of timerlist*/

static void PrintTimerList(INT call)
{
	TIMER *timer = timerList;
	INT i = 0;
	printf("----start call from %d---\n", call);
	for (; timer; timer = timer->nextTimer, i++)
	{
		printf("timer %d = 0x%x\n", i, timer->hwnd);
	}
	printf("----end call-------------\n", call);	
...
//printf("SETTIMER CALLED\n");
	dwTime = GetTickCount();
	for (; timer; prevTimer = timer, timer = timer->nextTimer)
	{
		if (timer->hwnd == hwnd && timer->idTimer == idTimer)
		{
			/*found existing timer, update parameters*/
			timer->uTimeout = uTimeout;
			timer->dwClockExpires = dwTime + uTimeout;
			timer->lpTimerFunc = lpTimerFunc;			
			return timer->idTimer;
		}
	}

	/*did not find existing timer, create new one*/
...
KillTimer(HWND hwnd, UINT idTimer)
{
	...
	for (; timer; prevTimer = timer, timer = timer->nextTimer)
	{
		if(timer->hwnd == hwnd && timer->idTimer == idTimer) {
			if (timer == timerList)
				timerList = timer->nextTimer;
			else
				prevTimer->nextTimer = timer->nextTimer;
...
MwHandleTimers(void)
{
	...
	dwTime = GetTickCount();
	for (timer = timerList; timer; timer = timer->nextTimer)
	{
...
SetLayeredWindowAttributes(HWND hwnd, COLORREF crKey, BYTE bAlpha, DWORD dwFlags)
{
	if (hwnd)
	{
		hwnd->bAlpha = (dwFlags & LWA_ALPHA) ? bAlpha : 0xFF;
		hwnd->dwFlags = dwFlags;
		...
		if (dwFlags & LWA_COLORKEY)
		{
			...
			/*enable colorkeying*/
			hwnd->DFB_window->SetOptions(hwnd->DFB_window, options);
			
			/*set the transparent color key */
			hwnd->DFB_window->SetColorKey(hwnd->DFB_window, GetRValue(crKey), GetGValue(crKey), GetBValue(crKey));
			
			/* set the alpha bytes to be transparent for transparent window*/
			hwnd->crKey = 0x00FFFFFF & crKey;
		}
		else
		{
			/* set the alpha bytes to be non transparent for transparent window*/
			hwnd->crKey = 0xFF000000 | crKey;
		}
		return 1;
	}
	return 0;
...
GetLayeredWindowAttributes(HWND hwnd, COLORREF *pcrKey, BYTE *pbAlpha, DWORD *pdwFlags)
{
	if (hwnd)
	{
		...
		if (pbAlpha)
			*pbAlpha = hwnd->bAlpha;
		if (pdwFlags)
			*pdwFlags = hwnd->dwFlags;
		return 1;
	}
	return 0;
}		

MWSCROLLBARINFO *GetVScrollInfo(HWND hwnd)
{
	return &hwnd->vscroll;
}
