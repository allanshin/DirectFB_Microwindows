...
extern void DFB_DrawOn(HWND, HDC);
extern void DFB_DrawOff(HWND, HDC);
...
{
/*
	if (!IsLayeredPaintStarted()) 			// Is this a layered window?					
	{
	...
	if(hdc != cliphdc) {
		...
		if(hdc->psd->flags&PSF_MEMORY) {
			...
			if (hdc->region != NULL && hdc->region->rgn != NULL
				...
				/*For memory DC's use existing clipregion, 
				 * not just the extents of the clip region
				 */
				LPRECT rptr = &(hdc->region->rgn->extents);
	
				MWCLIPREGION *vis;

				vis = GdAllocRectRegion(rptr->left, rptr->top,
	...
	LPCVOID lpszString, UINT cbCount, CONST INT *lpDx, int flags)
{
	...
	if((hdc->textalign & TA_BASELINE) == TA_BASELINE) {
		...
		flags |= MWTF_TOP;
//directfb font render
#if HAVE_DFBFONT_SUPPORT
	{
		PMWFONT pfont = hdc->font->pfont;
	  	pfont->fontprocs->DrawText(pfont, (PSD)hdc, x, y, lpszString, cbCount, flags);
	}
#else
...
CreateSolidBrush(COLORREF crColor)
{
	...
	hbr->style = BS_SOLID;
	//directfb
	if (crColor & 0xFF000000)
		hbr->color = 0x00FFFFFF & crColor; //mask out the alpha
	else
		hbr->color = 0xFF000000 | crColor;
	return (HBRUSH)hbr;
}
...
int SetDIBitsToDevice(HDC hdc,   int XDest,  int YDest,   DWORD dwWidth,  DWORD dwHeight,   
  int XSrc,   int YSrc,       UINT uStartScan,   UINT cScanLines,       const VOID *lpvBits,  
  const BITMAPINFO *lpbmi,  UINT fuColorUse)
{
	MWIMAGEHDR image;
	MWPALENTRY* psDestPalette;
	RGBQUAD* psSrcPalette;
	int i;
	DWORD YOffset;
...
/* image height in pixels*/
	image.height = dwHeight;
/* # image planes*/
	image.planes = 1;
/* bits per pixel (1, 4 or 8)*/
	image.bpp = lpbmi->bmiHeader.biBitCount;
...
/*	
		case 8:
			image.pitch = lpbmi->bmiHeader.biWidth;
			break;
		case 32:
			image.pitch = lpbmi->bmiHeader.biWidth*4;
			break;
		case 24:
			image.pitch = lpbmi->bmiHeader.biWidth*3;
			break;
*/		 
		case 4:
			image.pitch = PIX2BYTES(lpbmi->bmiHeader.biWidth<<2);
			break;
		case 1:
			image.pitch = PIX2BYTES(lpbmi->bmiHeader.biWidth);
			break;
/* for 8, 16, 24, 32 - bits	*/			
		default:
			image.pitch = (image.bpp / 8) *  lpbmi->bmiHeader.biWidth;
			break;
	}

// number of bytes per line (pitch) must be aligned to the 32-bits boundary
// (in accordance to the DIB format)
 	image.pitch = (image.pitch + 3) & (~3);
	
/* compression algorithm*/
	image.compression = ((image.bpp == 24) ||(image.bpp == 32)) ? MWIMAGE_BGR : MWIMAGE_RGB;
	if (lpbmi->bmiHeader.biHeight > 0)
		image.compression |= MWIMAGE_UPSIDEDOWN;
		
/* palette size*/
	if (image.bpp > 8)
		image.palsize = 0;
	else
		image.palsize = lpbmi->bmiHeader.biClrUsed ? lpbmi->bmiHeader.biClrUsed : (1 << image.bpp);
	
	image.transcolor = -1;	/* transparent color or -1 if none*/
	
// transforming color palette to MW format	
	psDestPalette = image.palette = (MWPALENTRY* )  malloc(image.palsize * sizeof (MWPALENTRY));
	psSrcPalette = (RGBQUAD*) lpbmi->bmiColors;
	for (i = image.palsize; i; i--, psDestPalette++, psSrcPalette++)
	{
		psDestPalette->r = psSrcPalette->rgbRed;
		psDestPalette->g = psSrcPalette->rgbGreen;
		psDestPalette->b = psSrcPalette->rgbBlue;	
	}
	
	
//	image.imagebits = (MWUCHAR *) lpvBits;	/* image bits (dword right aligned)*/
// take into account thes source start positions
//	YOffset = (lpbmi->bmiHeader.biHeight > 0) ? (lpbmi->bmiHeader.biHeight - dwHeight - YSrc) : (-lpbmi->bmiHeader.biHeight - dwHeight - YSrc);	//YSrc;
	YOffset = abs(lpbmi->bmiHeader.biHeight) - dwHeight - YSrc;
	
/* image bits (dword right aligned)*/
	image.imagebits = ((MWUCHAR *) lpvBits) + ((XSrc * image.bpp / 8) + YOffset * image.pitch);	/* image bits (dword right aligned)*/
//    printf ("\n\n***************\t imagebits address = %d compr = %d\n\n", (DWORD) image.imagebits, image.compression);

	DrawDIB(hdc, XDest, YDest, &image);
                        ...
                        CONST BITMAPINFO * pbmi, UINT fUsage)
{
     ...
     if (fInit == CBM_INIT)
     {
			...
			SelectObject (hdcMem, hBitmap) ;
           	SetDIBitsToDevice (hdcMem, 0, 0, pbmih->biWidth, pbmih->biHeight, 0, 0, 0, pbmih->biHeight, pBits, pbmi, fUsage) ;
           	DeleteDCOnly (hdcMem) ;
     }
	 else
          hBitmap = CreateCompatibleBitmap (hdc, pbmih->biWidth , pbmih->biHeight) ;
...
BOOL FillRgn(HDC hdc, HRGN hrgn, HBRUSH hbr)
{
	...
	SelectClipRgn(hdc, hrgn);	// mine
	GetClientRect(hdc->hwnd, &rc);
	return (BOOL) FillRect(hdc, &rc, hbr);
...
BOOL PaintRgn(HDC hdc, HRGN hrgn)
{
	...
	return FillRgn(hdc, hrgn, (HBRUSH) hdc->brush);
}
...
Parameters:
hgdiobj [in]  - Handle to the graphics object of interest. This can be a handle to one of the following: a logical bitmap, a brush, a font, a palette, a pen, or a device independent bitmap created by calling the CreateDIBSection function. 
...
{
// TO DO: implement for other object types, if it'll be requested
	 switch (hgdiobj->hdr.type)
	 {
	 	case OBJ_BITMAP:
			if (lpvObject)
			{
				PBITMAP pBitmap = (PBITMAP) lpvObject;
				MWBITMAPOBJ* pBitmapObj = (MWBITMAPOBJ*)hgdiobj; 
				pBitmap->bmType = 0;
				pBitmap->bmWidth = pBitmapObj->width; 
				pBitmap->bmHeight = pBitmapObj->height; 
			// in MW for bpp <= 8, linelen is in bytes per line
			// 		for bpp	> 8, ilnelen is in pixels
				pBitmap->bmWidthBytes = (pBitmapObj->bpp <= 8) ?
					pBitmapObj->linelen: pBitmapObj->linelen * (pBitmapObj->bpp / 8) ; 
				pBitmap->bmPlanes = pBitmapObj->planes; 
				pBitmap->bmBitsPixel = pBitmapObj->bpp; 
				pBitmap->bmBits = pBitmapObj->bits;
