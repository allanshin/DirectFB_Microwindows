...
static void ResetColorCache(void);
static void GdSortPalette(MWPALENTRY *pal, int size);
...
GdOpenScreen(void)
{
	...
	GdSortPalette(stdpal, psd->ncolors);
#if xxxALPHABLEND
...
typedef struct COLORCACHE* PCOLORCACHE;
...
#define NUM_COLORLIST_ITEMS		16

COLORCACHE *ColorCache[NUM_HASH_ENTRIES];

inline MWPIXELVAL MWPixelValue(MWPALENTRY* PalEntry)
{
//	return ((((PalEntry->r)>>3)<<10) | (((PalEntry->g)>>3)<<5) | ((PalEntry->b)>>3));
	return (((unsigned int)(PalEntry->r)<<16) | ((unsigned int)(PalEntry->g)<<8) | (unsigned int)(PalEntry->b));
}

#if 0
static void PrinColorCacheList(void)
{
	int i;
	COLORCACHE *current, *next;
	...
	for (i = 0; i < NUM_HASH_ENTRIES; i++)
	{
		...
		{
			printf("%2d: ", i); 
			current = ColorCache[i];
			while (current)
			{
				printf("0x%x ", current->cr); 
...
static void PrintPalette(MWPALENTRY *pal, int size)
{
	if (pal)
	{
		int i;
		...
		{
			MWCOLORVAL colorVal;
			colorVal = RGB2PIXEL888(pal[i].r, pal[i].g, pal[i].b);
			printf("%d\n", colorVal);
		}
	}
}
...
static void GdSortPalette(MWPALENTRY *pal, int size)
{
	if (pal)
	{
		...
		for (i = 1; i < size; i++)
		{
			MWPALENTRY temp;
			temp = pal[i];
			col = RGB2PIXEL888(pal[i].r, pal[i].g, pal[i].b);
			j = i;
			while ((j > 0) && ((RGB2PIXEL888(pal[j-1].r, pal[j-1].g, pal[j-1].b)) > col))
			{
				pal[j] = pal[j-1];
				j = j - 1;
			}
...
static
COLORCACHE **GetColorListPtrAddr(MWCOLORVAL cr)
{
	int index;
	
	/* simple hash function*/
	index = cr % NUM_HASH_ENTRIES;
...
ResetColorCache(void)
{
	...
	/* Free all memory of hash table link lists */
	for (i = 0; i < NUM_HASH_ENTRIES; i++)
	{
		if (ColorCache[i])
		{
...
void
MoveColorToHead(COLORCACHE **head, COLORCACHE *prevColor, COLORCACHE *color)
{
	if (head == NULL || prevColor == NULL || color == NULL)
		return;
		
	/* check if color is already at head position */
	if (color == *head)
		return;

	prevColor->next = color->next;
	color->next = *head;
	*head = color;
}

/* Most recently used color is in the front of each list*/
int
CheckColorCache(MWCOLORVAL cr)
{
	COLORCACHE **head, *color, *prevColor;
	head = GetColorListPtrAddr(cr);
	
	color = prevColor = *head;
			
	for (; color; prevColor = color, color = color->next)
	{
		if (color->cr == cr)
		{
//			printf("-- HIT cache ");	
			MoveColorToHead(head, prevColor, color);
			return color->best;
		}
	}

//	printf("--miss cache 0x%x\n", cr);	
	/* cache miss */
	return -1;
}
...
/* Most recently used color is in the front of each list*/
void
UpdateColorCache(MWCOLORVAL cr, MWPIXELVAL best)
{
	int itemCount = 0;
	COLORCACHE **head, *color, *prevColor;
	head = GetColorListPtrAddr(cr);
	...
	while (color)
	{
		if (color->cr == cr)
		{
			/* color is already in cache, just move it to beginning */
			MoveColorToHead(head, prevColor, color);
			return;
		}
	...
	/* cache miss. If list < NUM_COLORLIST_ITEMS then grow list, 
	 * if not, replace the last entry item data*/
	 
	if (itemCount < NUM_COLORLIST_ITEMS - 1)
	{
		COLORCACHE *temp;
		if (!(temp = (COLORCACHE *)malloc(sizeof(COLORCACHE))))
		{
			return;
		}
		temp->cr = cr;
	...
	{
		/* Put valid data into the last element */
		color->cr = cr;
		color->best = best;
...
int CheckColorPalette(MWPALENTRY *pal, int size, MWCOLORVAL cr)
{
	int first, mid, last;
	MWCOLORVAL temp;
	...
	/*binary search the sorted palette to find color entry*/
	while (first <=last)
	{
		mid = (first + last) >> 1;
		temp = RGB2PIXEL888(pal[mid].r, pal[mid].g, pal[mid].b);
		if (cr > temp)
			first = mid + 1;
		else if (cr < temp)
			last = mid - 1;
	...
	/* color not found in palette.  Palette miss*/
	return -1;
}
