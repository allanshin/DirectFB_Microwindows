...
#define USE_OFFSCREEN_BUFFER 1		/*ashin 01/30/06: 
									=1 for render to offscreen buffer first, 
									then blit to screen device during idle time*/
#define USE_OFFSCREEN_BUFFER_REGIONS 1 	/*ashin 01/31/06: 
											=1 for using update rectangles rather than
											copying over entire buffer every update*/

#define SWAP_OSD_BUFFERS 0		/*	ashin 05/01/06: when using OSD buffers, choose
									to first swap offscreen buffer with screen buffer
									first, then copy back sychronizing data after update is
									complete */
...
typedef struct _mwscreendevice {
	...
	void 	(*StretchBlitEx) (PSD dstpsd, PSD srcpsd,
			...
			long op);
	void (*DrawImage)(PSD psd);
	void	(*BatchBlit)(PSD destpsd);
	void (*Flush)(void);
	void (*InitPaletteLookup)(void);
	MWPIXELVAL (*FindNearestColor)(MWCOLORVAL color);
} SCREENDEVICE;
