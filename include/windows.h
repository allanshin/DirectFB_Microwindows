...
struct hwnd {
	...
	COLORREF 	crKey;			/* layered windows color key*/
	BYTE		bAlpha;			/* layered windows alpha value*/
	DWORD		dwFlags;		/* layered windows draw flag*/
	char		shouldPaint;	/* layered windows opaque flag*/
	char		gotClearRegionMsg;	/* window had region clear PostMessage*/\
/* DirectFB stuff */
	RECT					DFB_winrect;
	IDirectFBWindow			*DFB_window;
	IDirectFBSurface		*DFB_windowSurface;
	void *					DFB_addr;
	int						DFB_linelen;
	BOOL					DFB_SurfaceLocked;
